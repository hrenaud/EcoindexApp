...
function createDMG(appPath, dmgPath, appName) {
  ...
  execSync(`hdiutil create -volname "${appName}" -srcfolder "${dmgVolume}" -ov -format UDZO -fs HFS+ "${dmgPath}"`, { stdio: 'inherit' });
...
function findLatestApp() {
  try {
    if (!existsSync(zipDir)) {
      console.error(`Le répertoire ${zipDir} n'existe pas. Lancez d'abord 'npm run make'.`);
      process.exit(1);
    }
    ...
    for (const archDir of archDirs) {
      ...
      const zipFiles = files.filter(f => f.endsWith('.zip') && !f.includes('temp'));
      
      if (zipFiles.length > 0) {
        // Prendre le dernier ZIP
        const zipFile = join(archPath, zipFiles[zipFiles.length - 1]);
        const appName = zipFiles[zipFiles.length - 1].replace('.zip', '').replace(`-${archDir}`, '');
